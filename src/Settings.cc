...
#include "CameraModels/Fovmodel.h" 
#include "System.h"
...
namespace ORB_SLAM3 {
    ...
    void Settings::readCamera1(cv::FileStorage &fSettings) {
        ...
        if (cameraModel == "PinHole") {
        ...
        else if (cameraModel == "FovModel") {
            cameraType_ = Fov_Model;
            ...
            float cx = readParameter<float>(fSettings,"Camera1.cx",found);
            float cy = readParameter<float>(fSettings,"Camera1.cy",found);
            float w = readParameter<float>(fSettings,"Camera1.w",found);
            // float xi = readParameter<float>(fSettings,"Camera1.xi",found);

            vCalibration = {fx, fy, cx, cy, w};

            calibration1_ = new Fovmodel(vCalibration);
            originalCalib1_ = new Fovmodel(vCalibration);

            //Check if it is a distorted Fov
            readParameter<float>(fSettings,"Camera1.w",found,false);
            if(found){
                vPinHoleDistorsion1_.resize(1);
                vPinHoleDistorsion1_[0] = readParameter<float>(fSettings,"Camera1.w",found);
            }
            // Check if we need to correct distortion from the images
    ...
    void Settings::readCamera2(cv::FileStorage &fSettings) {
        ...
        if (cameraType_ == PinHole) {
            ...
            float cx = readParameter<float>(fSettings,"Camera2.cx",found);
            float cy = readParameter<float>(fSettings,"Camera2.cy",found);
            // float xi = readParameter<float>(fSettings,"Camera2.xi",found);
            float w  = readParameter<float>(fSettings,"Camera2.w",found);

            vCalibration = {fx, fy, cx, cy, w};

            calibration2_ = new Fovmodel(vCalibration);
            originalCalib2_ = new Fovmodel(vCalibration);

            //Check if it is a distorted Fov
            readParameter<float>(fSettings,"Camera2.w",found,false);
            if(found){
                vPinHoleDistorsion1_.resize(1);
                vPinHoleDistorsion1_[0] = readParameter<float>(fSettings,"Camera2.w",found);
            }
            ...
            if(sensor_ == System::STEREO || sensor_ == System::IMU_STEREO){
                ...
                static_cast<Fovmodel*>(calibration1_)->mvLappingArea = vOverlapping;
            }
        }
    ...
    void Settings::precomputeRectificationMaps() {
        ...
        cv::Mat P1, P2, Q;
        if(cameraType_ == Fov_Model){
            cv::stereoRectify(K1,cameraFov1DistortionCoef(),K2,cameraFov2DistortionCoef(),newImSize_,
                            R12, t12,
                            R_r1_u1,R_r2_u2,P1,P2,Q,
                            cv::CALIB_ZERO_DISPARITY,-1,newImSize_);
            cv::initUndistortRectifyMap(K1, cameraFov1DistortionCoef(), R_r1_u1, P1.rowRange(0, 3).colRange(0, 3),
                                        newImSize_, CV_32F, M1l_, M2l_);
            cv::initUndistortRectifyMap(K2, cameraFov2DistortionCoef(), R_r2_u2, P2.rowRange(0, 3).colRange(0, 3),
    ...
    ostream &operator<<(std::ostream& output, const Settings& settings){
        ...
        else if(settings.cameraType_ == Settings::Fov_Model || settings.cameraType_ ==  Settings::Fov_Model){
            output << "FovModel";
        }
        else{
        ...
        if(settings.sensor_ == System::STEREO || settings.sensor_ == System::IMU_STEREO){
            ...
            else if(settings.cameraType_ == Settings::Fov_Model){
                output << "FovModel";
            }
            else{
                output << "Kannala-Brandt";
